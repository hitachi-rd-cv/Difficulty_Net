...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.dataset == 'cifar10' or config.dataset == 'cifar100':
        ...
        if config.backbone == 'resnet10_fe':
            feat_in = 512
        else:
            feat_in = 2048
    ...
    if config.dataset != 'places':
        block = None
    for tau in np.linspace(0, 2, 41):
        top1, ece = validate(val_loader, model, classifier, lws_model, criterion, config, logger, block, tau=tau)


def validate(val_loader, model, classifier, lws_model,  criterion, config, logger, block=None, tau = 0):
    ...
    progress = ProgressMeter(
        ...
        prefix='Eval: ')
    classifier_weights = classifier.module.fc.weight.data
    avgpool = nn.AdaptiveAvgPool2d((1, 1))
    normB = torch.norm(classifier_weights, 2, 1,)
    ws = classifier_weights.clone()
    for i in range(classifier_weights.size(0)):
