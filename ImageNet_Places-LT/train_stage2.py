...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.resume:
        if os.path.isfile(config.resume):
            ...
            #classifier.fc.weight.data = checkpoint['state_dict_classifier'].fc.weight.data
            if config.dataset == 'places':
    ...
    criterion = LabelAwareSmoothing(cls_num_list=cls_num_list, smooth_head=config.smooth_head,
                                    smooth_tail=config.smooth_tail).cuda(config.gpu)   ### label-aware smoothing criterion

    #criterion = nn.CrossEntropyLoss().cuda(config.gpu)

    optimizer = torch.optim.SGD([{'params': lws_model.parameters()}, ], config.lr,  #{"params": classifier.parameters()},{'params': lws_model.parameters()}
    ...
    for epoch in range(config.num_epochs):
        ...
        acc1 = validate(val_loader, model, classifier, lws_model, criterion, config, logger, block)
...
def train(train_loader, model, classifier, lws_model, criterion, optimizer, epoch, config, logger, block=None):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if mixup is True:
            images, targets_a, targets_b, indx, lam = mixup_data(images, target, alpha=config.alpha)
        with torch.no_grad():
            if config.dataset == 'places':
